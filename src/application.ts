...
import {ThrizerApiBindings} from './keys';
export class ThrizerApplication extends BootMixin(
  ...
  setUpBindings(options: ApplicationConfig): void {
    this.bind('datasources.config.ApiDS').to(options.thrizer?.db);
    this.bind(ThrizerApiBindings.CONFIG).to(options);
